...
本書を通して、頑強で再現可能な方法を強調する。頑強（robust）とは、出力されないエラー、交絡因子、ソフトウェアのバグ、ノイズの多いデータに対する耐性を意味する。再現可能（reproducible）とは、他人が作業を繰り返して同じ結果に到達できることを意味する。そのためには、作業を記録し、全ての材料（方法、コード、データ）を利用可能にしなければならない。Chapter 2で詳しく述べる。
